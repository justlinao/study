...
if __name__ == '__main__':
    path = os.getcwd()
    case_path = os.path.dirname(path) + '/Test_case'
    discover = unittest.defaultTestLoader.discover(case_path,
                                                   pattern="test*.py",
                                                   top_level_dir=None)   # 测试模块的顶层目录，如果没有顶层目录，默认为None
    with open(os.path.dirname(path)+'/Test_report/HTMLTestRunner.html', 'wb+') as f:
        ...
        runner.run(discover)
    get_report = get_report(os.path.dirname(path)+'/Test_report')
